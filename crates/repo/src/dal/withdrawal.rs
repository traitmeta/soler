use ::entities::withdrawals::{ActiveModel, Column, Entity, Model};
use sea_orm::*;

pub struct Query;

impl Query {
    pub async fn find_by_height(db: &DbConn, hash: Vec<u8>) -> Result<Option<Model>, DbErr> {
        Entity::find()
            .filter(Column::BlockHash.eq(hash))
            .one(db)
            .await
    }
}
