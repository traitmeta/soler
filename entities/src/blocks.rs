...
use sea_orm::{entity::prelude::*, EntityOrSelect, QueryOrder, QuerySelect};
...
impl Entity {
    pub fn find_latest() -> Select<Entity> {
        Self::find().order_by_desc(Column::Number).limit(1)
    }

    pub fn find_by_hash(hash: &str) -> Select<Entity> {
        Self::find().filter(Column::Hash.eq(hash))
    }
}
